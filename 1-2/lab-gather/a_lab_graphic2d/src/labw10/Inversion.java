...
public class Inversion extends JFrame{
	...
	public Inversion() {
		try {
			...
			//convert to negative
			for (int y = 0; y < height; y++) {  //loop through each pixel in image
	            for (int x = 0; x < width; x++) { 
	                int p = image.getRGB(x, y); //use bitwse
	                int a = (p >> 24) & 0xff; 
	                int r = (p >> 16) & 0xff; 
	                int g = (p >> 8) & 0xff; 
	                int b = p & 0xff; 
	  
	                // subtract RGB from 255 to invert rgb
	                r = 255 - r; 
	                g = 255 - g; 
	                b = 255 - b; 
	  
	                // set new RGB value 
	                p = (a << 24) | (r << 16) | (g << 8) | b; //recombine the rgb back
	                image.setRGB(x, y, p); 
				}
			}
