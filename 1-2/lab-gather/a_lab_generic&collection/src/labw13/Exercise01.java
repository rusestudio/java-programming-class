...
class MyArrayAlg {
	...
	public static <T extends Comparable<T>> T getMin(T[] arr) { 
		//T must implement Comparable to be able to use comparaTo
	...
	public static <T> void reverse(T[] arr) {
		    int left = 0;             //start at first index
		    int right = arr.length - 1;  //start at last index = n-1
		    ...
		    while (left < right) {
		        ...
		        left++;
		        right--;
		        //stop when left n right at the same point in odd case
		    }
	}
	...
	public static <T extends Comparable<T>> void sort(T[] arr) {
		...
		for(int i =0 ; i < arr.length; i++) {
			for(int j = 0; j < arr.length; j++) {
				if(arr[i].compareTo(arr[j]) < 0) { //index 0 start compare to sort
					T temp = arr[i];
					arr[i] = arr[j];
					arr[j] =temp;
				}
