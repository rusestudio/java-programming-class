...
public class BankSystem {
	...
	public void process(int idx) {
		...
		if(idx==0) {
			...
			if (withdrawamt > loggedIn.getBalance()) {
				System.out.println("잔액이 부족하거나 잘못된 금액을 입력하였습니다.");
			} else {
				loggedIn.withdraw(withdrawamt);
			...
			if (transferamt > loggedIn.getBalance()) {
				System.out.println("잔액이 부족하거나 잘못된 금액을 입력하였습니다.");
			} else {
				loggedIn.transfer(targetAccount, transferamt);
